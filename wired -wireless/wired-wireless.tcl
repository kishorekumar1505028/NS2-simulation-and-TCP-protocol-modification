...
#Class AddrParams is used to store the topology hierarchy like
 #number of levels of hierarchy,
 #number of areas in each level like number of domains,
 #number of clusters in each domain
 #number of nodes in each cluster
  AddrParams set domain_num_ 3 ; #number of domains in each level
 #create a list called cluster_num          
  lappend cluster_num 2 1 1 ; #2 clusters in D1 , 1 in D2 , 1 in D3 => total 4             
  AddrParams set cluster_num_ $cluster_num
  lappend eilastlevel 1 1 4 1               ; #1 node in C1 , 1 in C2 , 4 in C3 , 1 in C4
  AddrParams set nodes_num_ $eilastlevel 
##################################necessary files
  set tracefd  [open $opt(tr) w]
  $ns_ trace-all $tracefd
  set nam_file wired-and-wireless.nam
  set namtracefd [open $nam_file w]
  ...
  #create wired nodes
  set temp {0.0.0 0.1.0}  ;#temp[0] = 0.0.0 temp[1] = 0.1.0       
  ...
  set BS(1) [$ns_ node 2.0.0] ; # a HierNode is created with an address of 2.0.0;
  ...
  proc finish {} {
	global ns_ tracefd namtracefd nam_file
